...
export default function AuthNav() {
  ...
  return (
    <nav className="max-w-4xl mx-auto flex items-center justify-between gap-3 py-2">
      <div className="flex min-w-0 items-center gap-3 sm:gap-4">
        <Link href="/" className="truncate font-semibold text-base text-slate-100 sm:text-lg">All API Dashboard</Link>
        <div className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-[10px] font-medium uppercase tracking-wider text-slate-400">
      ...
      <div className="flex items-center gap-4">
        {loading ? (
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link href="/dashboard" className="text-sm font-medium text-slate-300 hover:text-indigo-400 transition-colors">Dashboard</Link>
            <Link href="/shop" className="text-sm font-medium text-slate-300 hover:text-indigo-400 transition-colors">Shop</Link>
            <div className="flex items-center gap-3">
              {avatarUrl ? (
                ...
                <div aria-hidden className="h-8 w-8 rounded-full bg-slate-700 flex items-center justify-center text-sm font-medium text-slate-300">
                  {String(name).split(" ").map((s: string) => s[0]).slice(0, 2).join("").toUpperCase()}
                </div>
              )}
